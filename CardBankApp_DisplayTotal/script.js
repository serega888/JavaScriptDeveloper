...
//-----------------------First way---------------------------
const userName = 'Sergey Gray'; // nickname = 'sg'
...
//-----------------------Second way-----------------------------
const userNameTwo = 'Sergey Blue';
const nickNameTwo = userNameTwo
  .toLowerCase()
  .split(' ')
  .map(item => item[0])
  .join('');
...
const createNickNamesTwo = function (accs) {
  accs.forEach(
    acc =>
      (acc.nickNameTwo = acc.userName
        .toLowerCase()
        .split(' ')
        .map(function (item) {
          return item[0];
...
createNickNamesTwo(accounts);
console.log(accounts);

//--------------------Third way---------------------------------

const createNickNamesThree = accs =>
  accs.forEach(
    acc =>
      (acc.nickNameThree = acc.userName
        .toLowerCase()
        .split(' ')
        .map(function (item) {
          return item[0];
...
const displayTotal = function (transactions) {
  ...
  const interestTotal = transactions
    .filter(trans => trans > 0)
    .map(depos => (depos * account1.interest) / 100)
