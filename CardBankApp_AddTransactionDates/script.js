...
const displayTransactions = function (account, sort = false) {
  ...
  transactionsSorted.forEach(function (trans, index) {
    ...
    const monthTrans = `${date.getMonth() + 1}`.padStart(2, '0');
    const yearTrans = date.getFullYear();

    const transDate = `${dayTrans}/${monthTrans}/${yearTrans}`;
...
btnLogin.addEventListener('click', function (e) {
  ...
  const dayNow = `${dateNow.getDay()}`.padStart(2, '0');
  const monthNow = (dateNow.getMonth() + 1).toString().padStart(2, '0');
  const yearNow = dateNow.getFullYear();
  labelDate.textContent = `${dayNow}/${monthNow}/${yearNow}`;
