...
//----------------------------------------------Array.from() get datas from HTML----------
const logoCard = document.querySelector('.logo');

logoCard.addEventListener('click', function (e) {
  e.preventDefault();

  const transactionsUserInterface = document.querySelectorAll(
    '.transactions__value'
  );
  console.log('transactionsUserInterface', transactionsUserInterface);
  const arrTransactionsUserInterface = Array.from(transactionsUserInterface);
  console.log(
    "arrTransactionsUserInterface.map(arr =>Number(arr.textContent.split('$')[0]))",
    arrTransactionsUserInterface.map(arr =>
      Number(arr.textContent.split('$')[0])
    )
  );
});

//---------------------------or--------------------------------
const logoCardTwo = document.querySelector('.logo');

logoCardTwo.addEventListener('click', function () {
  const transactionsUserInterfaceTwo = document.querySelectorAll(
    '.transactions__value'
  );
  const allTransactionArraysTwo = Array.from(
    transactionsUserInterfaceTwo,
    arr => arr.textContent.split('$')[0]
  );
  console.log('allTransactionArraysTwo', allTransactionArraysTwo);
  console.log(
    "Array.from(transactionsUserInterfaceTwo,arr => arr.textContent.split('$')[0])",
    Array.from(
      transactionsUserInterfaceTwo,
      arr => arr.textContent.split('$')[0]
    )
